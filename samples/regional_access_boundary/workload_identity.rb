...
def main
  ...
  if ENV["GOOGLE_APPLICATION_CREDENTIALS"] &&
     ENV["GOOGLE_APPLICATION_CREDENTIALS"] != File.expand_path("workload_identity_config.json", __dir__)
    mock_mode = false
  end
  ...
  if mock_mode
    ...
    config_path = File.expand_path "workload_identity_config.json", __dir__
  ...
  if mock_mode
    ...
    # 3. Stub the RAB lookup endpoint for Workload Identity
    url = "https://iamcredentials.googleapis.com/v1/projects/123456/locations/global/workloadIdentityPools/POOL_ID/allowedLocations"
  ...
  if x_allowed_locations && x_allowed_locations != 'STILL NOT PRESENT'
    puts "Success! RAB header is present for workload identity: #{x_allowed_locations}"
  else
    puts "Failure! RAB header should be present for workload identity."
  end
