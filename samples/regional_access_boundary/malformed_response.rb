...
def main
  ...
  if credentials.is_a? Google::Auth::ServiceAccountCredentials
    ...
    stub_request(:get, url)
      .to_return(status: 200, body: JSON.generate({ "locations" => ["us-central1"] }))
  ...
  puts "\nSleeping for 2 seconds to let background fetch fail on malformed response..."
  sleep 2
  ...
  if x_allowed_locations
    puts "Failure! RAB header should NOT be present after malformed response."
  else
    puts "Success! RAB header is not present after malformed response."
  end
