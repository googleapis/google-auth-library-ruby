...
describe "RegionalAccessBoundary Integration" do
  ...
  describe "applying headers" do
    ...
    it "recovers from non-StandardError thread crash and does not get stuck fetching" do
      mock_fetcher = double "Fetcher"
      allow(Google::Auth::RegionalAccessBoundary::Fetcher).to receive(:new).and_return(mock_fetcher)
      allow(mock_fetcher).to receive(:fetch).and_raise(Exception, "Simulated thread crash")

      expect do
        client.apply! headers, url: url
      end.to raise_error(Exception, "Simulated thread crash")

      entry = cache.instance_variable_get(:@entries)[key]
      expect(entry.is_fetching).to be_falsey
      expect(entry.cooldown_expiry).to be_truthy # Cooldown is active
    end
